...
for i_reps in xrange(n_reps):
    ...
    x_old_cm = x_start_cm + rand_offset_x_cm
    y_old_cm = y_start_cm + rand_offset_y_cm
...
for i_wind in xrange(n_windows):
    if i_reps == 1:
        ...
        bar(i_wind * x_step_cm + x_start_cm, tot_spikes[i_reps, i_wind], width=x_step_cm, color=str( 0.5*(1 + frac_mincells + (1 - frac_mincells)*cos(i_wind / float(n_windows) * n_osc_cycles * 2*pi)) )) 
    else:
        ...
        bar(i_wind * x_step_cm + x_start_cm, nanmean(tot_spikes[:, i_wind]), width=x_step_cm, color=str( 0.5*(1 + frac_mincells + (1 - frac_mincells)*cos(i_wind / float(n_windows) * n_osc_cycles * 2*pi)) ))
...
ax.set_yticks([0, max(n_grid_cm, ylim()[1])])
ax.set_yticklabels([0, int(max(n_grid_cm, ylim()[1])) ])
for label in ax.get_xticklabels() + ax.get_yticklabels(): 
    label.set_fontsize(fsize) # 5
...
savefig('figs/plot_speedmod'+str(mod_depth)+'_sim_seq_paramsid_'+str(params_id)+'_'+str(n_reps)+'reps_'+str(int(n_mincells))+'mincells_'+str(n_windows)+'steps.png', dpi=300)

print "saving plot data..."
file_save = open('data/speedmod_'+str(mod_depth)+'_sim_seq_paramsid_'+str(params_id)+'_'+str(n_reps)+'reps_'+str(int(n_mincells))+'mincells_'+str(n_windows)+'steps.txt','w')
