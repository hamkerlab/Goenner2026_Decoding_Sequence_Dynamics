...
baseline = 1e-7 # 0.005 # 0.001 # 0.01 # 0.1 too high?! # 0.001 - 0.01 seems OK
...
for i_reps in xrange(n_reps):
    ...
    var_2Dstep_const_cm = st.lognorm.rvs(0.25, scale=step2D_cm)
    var_1Dstep_const_cm = var_2Dstep_const_cm / sqrt(2)
    print "var_1Dstep_const_cm = ", var_1Dstep_const_cm
    ...
    for i_wind in xrange(n_windows):
        ...
        osc_modulation = 0.5*(1 + frac_mincells + (1 - frac_mincells) * cos(osc_phase))   # = 0.5*( 1 + cos(osc_phase) + frac_mincells(1 - cos(osc_phase)) )
        n_cells = 200 # int(200 * osc_modulation )

        peak_rate_Hz = (1/sigma_scale_factor)**2 * 300.0 * osc_modulation

        x_true_array_cm[i_reps, i_wind] = x_start_cm + i_wind * var_1Dstep_const_cm
        y_true_array_cm[i_reps, i_wind] = y_start_cm + i_wind * var_1Dstep_const_cm
        ...
        virtualPF_xytrue = zeros(n_cells)
        ...
        while sum(n_spikes) == 0:
            ...
            virtualPF_xytrue[:] = exp(-((x_true_array_cm[i_reps, i_wind] - mu_x[:])**2 + (y_true_array_cm[i_reps, i_wind] - mu_y[:])**2)/sigma_code_cm**2) + baseline
            poi_rate[:] = peak_rate_Hz * DeltaT_window_sec * virtualPF_xytrue[:] * exp(- DeltaT_window_sec *  virtualPF_xytrue[:])
            n_spikes[:] = poisson(poi_rate, size=n_cells)
    ...
    if i_reps==0:
        ...
        for i_wind in xrange(n_windows):
            #plot(x_mean_array_cm[i_reps, i_wind] + R[i_wind] * cos(t), y_mean_array_cm[i_reps, i_wind] + R[i_wind]*sin(t), color=str(1 - tot_spikes[i_reps, i_wind] / max(tot_spikes)), lw=linew) # CI around the sample mean
...
for i_wind in xrange(n_windows):
    if i_reps == 1:
        bar(x_true_array_cm[i_reps, i_wind] - 0.5*x_step_cm, tot_spikes[i_reps, i_wind], width=x_step_cm, color=str( 0.5*(1 + frac_mincells + (1 - frac_mincells)*cos(i_wind / float(n_windows) * 4*pi)) )) # osc_modulation
    else:
...
savefig('figs/plot_vartotspeed_sequence_sigma_code_'+str(int(sigma_code_cm))+'_'+str(n_reps)+'reps_'+str(int(n_mincells))+'mincells_'+str(baseline)+'baseline_'+str(n_windows)+'steps.png', dpi=300)

print "saving plot data..."
file_save = open('data/sim_vartotspeed_sequence_sigma_code_'+str(int(sigma_code_cm))+'_'+str(n_reps)+'reps_'+str(int(n_mincells))+'mincells_'+str(baseline)+'baseline_'+str(n_windows)+'steps.txt','w')
