...
# ln(X) ist normalverteilt ~ N(mu, sigma^2) => X ist log-normalverteilt mit Median exp(mu) und Mittelwert exp(mu + 0.5 * sigma^2)
# median(st.lognorm.rvs(sigma, scale=2.5, size=1000)) # erzeugt lognormale ZVs mit Median 2.5 - Varianz bestimmt durch sigma
# Bsp: st.lognorm.rvs(0.25, scale=2.5) erzeugt haputsaechlich zuf.zahlen <= 2*Median
...
baseline = 1e-7 # 1e-6 # 1e-5 # 0.0001 # 0.005 # 0.001 # 0.01 # 0.1 too high?! # 0.001 - 0.01 seems OK (noticeable influence), 1e-7 is about the lowest
...
for i_reps in xrange(n_reps):
    ...
    for i_wind in xrange(n_windows):
        ...
        x_true_array_cm[i_reps, i_wind] = x_old_cm + var_1Dstep_const_cm * speed_modulation
        y_true_array_cm[i_reps, i_wind] = y_old_cm + var_1Dstep_const_cm * speed_modulation
...
savefig('figs/plot_speedmod'+str(mod_depth)+'_vartotspeed_sequence_sigma_code_'+str(int(sigma_code_cm))+'_'+str(n_reps)+'reps_'+str(int(n_mincells))+'mincells_'+str(baseline)+'baseline_'+str(n_windows)+'steps.png', dpi=300)

print "saving plot data..."
file_save = open('data/speedmod_'+str(mod_depth)+'_vartotspeed_sequence_sigma_code_'+str(int(sigma_code_cm))+'_'+str(n_reps)+'reps_'+str(int(n_mincells))+'mincells_'+str(baseline)+'baseline_'+str(n_windows)+'steps.txt','w')
